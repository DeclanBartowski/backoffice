...
@section('content')
    <div class="content-block">
        <form action="{{route('blocks.store')}}" method="POST" class="edit-form">
            @csrf
            <div class="field-item">
                <label for="blockName" class="field-name field-name-small">{{__('blocks.title')}}</label>
                <input type="text" name="name" class="field" required="" id="blockName" maxlength="100">
            </div>
            <div class="field-item">
                ...
                <div class="radio-group">
                    @foreach($rules as $key => $rule)
                        <label class="radio">
                            <input type="radio" name="rules" value="{{$key}}">
                            <span>{{$rule}}</span>
                        </label>
                    @endforeach
                </div>
